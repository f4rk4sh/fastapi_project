...
from app.schemas.schema_bank import BankResponse, BankCreate, BankUpdate
...
descriptions = CRUDEndpointsDescriptions(model_name="Bank", search_parameters=["name", "mfo"])
...
def search_banks(
    ...
    return bank.search(parameter, keyword, session, max_results)
...
@router.get("/bank/{bank_id}", status_code=status.HTTP_200_OK, description=descriptions.fetch_one)
@permission({ConstantRole.employer})
def fetch_bank(
