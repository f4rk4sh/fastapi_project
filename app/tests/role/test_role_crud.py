...
class TestCRUDGetRole:
    def test_successful_get_role(
        ...
        role_in_db = role.get(create_random_role.id)

        spy_role_get.assert_called_once_with(create_random_role.id)
        assert role_in_db.id == create_random_role.id
        assert role_in_db.name == create_random_role.name
    ...
    def test_failed_get_role(
        ...
        role_in_db = role.get(create_random_role.name)

        spy_role_get.assert_called_once_with(create_random_role.name)
        assert not role_in_db
...
class TestCRUDGetRoleByAttribute:
    def test_successful_get_role_by_attribute(
        ...
        role_in_db = role.get_by_attribute(name=create_random_role.name)

        spy_role_get_by_attribute.assert_called_once_with(name=create_random_role.name)
        assert role_in_db.id == create_random_role.id
        assert role_in_db.name == create_random_role.name
    ...
    def test_failed_get_role_by_attribute(
        ...
        role_in_db = role.get_by_attribute(name=create_random_role.id)

        spy_role_get_by_attribute.assert_called_once_with(name=create_random_role.id)
        assert not role_in_db
...
class TestCRUDSearchRoleByParameter:
    def test_successful_search_roles_by_parameter(
        ...
        roles_in_db = role.search_by_parameter(
            parameter="name", keyword=create_random_role.name
        )

        spy_role_search_by_parameter.assert_called_once_with(
            parameter="name", keyword=create_random_role.name
        )
        assert create_random_role in roles_in_db
    ...
    def test_failed_search_roles_by_parameter(
        ...
        roles_in_db = role.search_by_parameter(
            parameter=None, keyword=create_random_role.name
        )

        spy.assert_called_once_with(parameter=None, keyword=create_random_role.name)
...
class TestCRUDUpdateRole:
    def test_successful_update_role_from_schema(
        ...
        updated_role = role.update(
            role_in_db, RoleUpdate(id=create_random_role.id, name=new_name)
        )
    ...
    def test_successful_update_role_from_dict(
        ...
        new_name = random_string()
        updated_role = role.update(
            role_in_db, {"id": create_random_role.id, "name": new_name}
        )
    ...
    def test_failed_update_role(
        ...
        new_name = random_string()
        role.update(role_in_db, RoleUpdate(id=create_random_role.id, new_name=new_name))

        updated_role_in_db = role.get(create_random_role.id)

        spy_role_update.assert_called_once_with(
            role_in_db, RoleUpdate(id=create_random_role.id, new_name=new_name)
        )
...
class TestCRUDDeleteRole:
    def test_successful_delete_role(
        ...
        role.delete(create_random_role.id)
        role_in_db = role.get_by_attribute(id=create_random_role.id)

        spy_role_delete.assert_called_once_with(create_random_role.id)
        assert not role_in_db
    ...
    def test_failed_delete_role(
        ...
        role.delete(create_random_role.name)
        role_in_db = role.get_by_attribute(id=create_random_role.id)

        spy_role_delete.assert_called_once_with(id=create_random_role.id)
        assert role_in_db
