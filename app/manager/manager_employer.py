...
class EmployerManager(ManagerBase[Employer, CRUDEmployer, EmployerCreate, EmployerUpdate]):
    def create(self, obj_in: EmployerCreate):
        if self.executor.get_by_attribute(email=obj_in.user.email):
    ...
    def update(self, obj_in: EmployerUpdate):
        employer = self.executor.get(obj_in.id)
        ...
        return self.executor.update(employer, obj_in)

    def delete(self, id: int):
        employer = self.executor.get(id)
